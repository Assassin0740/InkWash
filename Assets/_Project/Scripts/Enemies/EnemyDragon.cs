...
namespace InkWash.Enemies
{
    ...
    public class EnemyDragon : EnemyBase
    {
        ...
        [Tooltip("★ 头部俯仰对齐（度，**抬头为正**）。只转末尾 `headAlignLinks` 节 ⇒ 头簇整块绕颈根转，脖子不动。\n" +
                 ...
                 "⇒ 3.2 = 吻部严格水平；30 ≈ **吻部上抬 25.8°、颅顶 54°**（用户：「头抬得这么高要干嘛」）。\n" +
                 "★ 旧值 30 出自第十八轮的「头簇 PCA 第一主轴俯仰 30.2°」—— 那把尺子量的是**颈+颅的质量走向**，" +
                 "不是吻部指向，属坑表条目 29。★ 头**不会**随行波俯仰（行波是纯偏航；实测 6 相位仰角极差 0.01°）" +
                 "⇒ 一个静态值就能完全定死，不需要动态补偿。")]
        public float headAlignPitchDeg = 3f;
