...
namespace InkWash.Enemies
{
    ...
    public class EnemyDragon : EnemyBase, InkWash.Effects.IDragonSpineSource
    {
        ...
        // ★★ 第二十八轮·终局（2026-09-19 用户定稿）：三轮自动反解全部宣告失败——
        //   ① 骨轴对齐（roll −65.46 组）→ 颅骨网格仍上仰；② 叶骨质心 → 须骨/下颌骨带偏，颅骨下折 40-50°；
        //   ③ 图像伺服（顶轮廓/PCA）→ 双视角目视"过关"后用户实测：「头完全反过来了」。
        //   ★ 根因共性：所有自动量测都被须骨、蒙皮顶点或视角欺骗，**机器看不见审美**。
        //   终值由用户 F9 调参面板（本文件底部 HeadTunerWindow）人眼定稿：
        //   **pitch=−55.5（抬头为正）/ yaw=0 / roll=0**（yaw/roll 用户未动，即原生姿态已对）。
        //   与 Z_Enemy_MoLong.prefab 保持一致（门禁 0 不一致）。
        public float headAlignPitchDeg = -55.5f;
        ...
        protected override void Update()
        {
            ...
            // ★ 第三十七轮：尸体坠地兜底统一走基类 TickDead → DeathSinkTick（本类私有版已删）
        }
        ...
        protected override void OnDied()
        {
            ...
            InkWash.Effects.DragonStormVfx.Shut();
            EndHover();
            // ★ 第三十七轮：坠地兜底统一走基类 TickDead → DeathSinkTick
            //   （普通怪/龙同一套；本类原有的私有实现删除，避免双重驱动打架。
            //    prefab 里 deathSinkDuration: 1.6 现在直接落基类同名字段）。
        }
    }
}
