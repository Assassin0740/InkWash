...
namespace InkWash.Player
{
    ...
    ///   Run        +0.049     +0.288     整段被抬高约 5cm（换片段后重量）
    ...
    ///
    /// ⚠ Run 的基准偏移**绑死在片段上**：换成 Kevin Iglesias Run01_Forward 后，
    ///   该状态的「无 IK 最低」从 +0.155 变成 +0.049，偏移必须跟着从 -0.155 改到 ≈-0.046。
    ///   忘了改 → 脚会穿地 10.6cm（姿态体检里「落脚状态脚不穿地」直接判未通过）。
    ///   换跑步/走路片段时，务必重跑 Tools/cs/s2_run_pose.cs 姿态体检并同步 s2_setup_combat.cs。
    ///
    /// 两类毛病方向相反，分别治：
    ///
