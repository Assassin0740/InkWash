// v39g_test.cs —— B 组终测：等敌 → dash逐帧 → 杀光 → 门涡+门下墨渍
...
IEnumerator Body()
{
    ...
    Debug.Log("[v39g] step-enemy-found"); sb.AppendLine("enemy = " + (enemy != null ? enemy.name : "NULL"));
    if (enemy == null) { Debug.Log("[v39g]\n" + sb); yield break; }
    ...
    if (pc != null && tr != null)
    {
        Debug.Log("[v39g] step-dash-inject"); pc.RequestInjectedDash(); Debug.Log("[v39g] step-dash-injected");
        var seq = "";
        for (int i = 0; i < 12; i++)
        {
            yield return null;
    ...
    Debug.Log("[v39g] step-killed"); sb.AppendLine("杀光耗时=" + sw1.ElapsedMilliseconds + "ms");
    ...
    Debug.Log("[v39g] step-door-wait-done"); yield return new WaitForSeconds(0.2f);
    run.DoorOpened -= onDoor;
