...
// drg_head21.cs —— 第二十一轮：把「吻部轴」尺子**锁死**（顶点索引固定），验证头部到底会不会随行波点头
//
// 为什么要锁（第二十轮的病）
...
//
// 出图：六档**绝对** pitch（含线上值 30），标签直接写绝对档位，用户指着哪张我就设哪个数。

public class drg_head21 : MonoBehaviour
{
    const string DIR = "D:/Unity Project/InkWash/Tools/screenshots/enemies/H21";
    const string RP = "D:/Unity Project/InkWash/Tools/reports/drg_head21.txt";
    const string KEY = "墨龙";
    const int W = 900, H = 600;
    const float DRIVE_HZ = 0.45f;

    static readonly float[] PHASES = new float[] { 0.00f, 0.1667f, 0.3333f, 0.50f, 0.6667f, 0.8333f };
    static readonly float[] ABS = new float[] { 30f, 5f, -5f, -15f, -23f, -30f };
    const float P_LEVEL = -23f;     // 第二十轮解出的「吻部水平」候选，Pass A2 用它复测
    ...
    IEnumerator Run()
    {
        ...
        foreach (float p in ABS)
        {
            ...
            Bake(); Measure();
            string tag = Tag(p);
            File.WriteAllBytes(DIR + "/h21_side_" + tag + ".png", Shot(_camS));
            File.WriteAllBytes(DIR + "/h21_q_" + tag + ".png", Shot(_camQ));
    ...
    IEnumerator Done()
    {
        ...
        Debug.Log("[drg_head21] 写入 " + RP);
        yield return null;
    }
}

var __hostH21 = new GameObject("drg_head21");
__hostH21.AddComponent<drg_head21>();
return "DRG_HEAD21_STARTED";
