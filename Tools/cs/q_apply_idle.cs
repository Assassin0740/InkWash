// 换 Idle 片段后重推 FootIK 偏移：Idle 由 -0.0003 改为 0.0307，其余 8 段保持 f_calib_footik 的重测值。
//
...
IEnumerator Body()
{
    ...
    System.Action flush = () => System.IO.File.WriteAllText(System.IO.Path.Combine(projRoot, "Tools/reports/q_apply_idle.txt"), sb.ToString());
    ...
    {
        { "Idle",     0.0307f },   // ← 2026-09-15 换 Feng 自带 Idle 后重测（原 -0.0003）
        { "Walk",     0.0569f },
        { "Run",     -0.2294f },
        { "Dash",    -0.0041f },
        { "Atk1",     0.0446f },
    ...
    flush();
    Debug.Log("[q_apply_idle] " + sb.ToString());
    yield return null;
}
return Body();
