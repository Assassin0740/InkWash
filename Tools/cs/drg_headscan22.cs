...
// drg_headscan22.cs —— 第二十二轮：**定档扫描**。尺子沿用第二十一轮（顶点索引锁死 + 空间锁死），
//
// 为什么要锁（第二十轮的病）
//   head20 每帧都**重新**按「平面切 + 前后百分位」选顶点：
//     · 头区 = `Dot(q − pivot, fwd) > −0.15` 的半空间
//     · 正中面 = `|lat| <= latMax*0.22` 的薄片
//   而 `pivot` 随**脖颈偏航**平移 ⇒ 切平面在头上来回扫 ⇒ **每帧选中的顶点集不一样**。
//   后果：Pass A 三个相位的「吻端簇展布」报告是 0.756 / 0.748 / 0.760 m，
//   恰好 ≈ 前向跨度本身 —— 一个 2% 薄片里的点怎么会横跨整颗头？**只能是被污染的集合**。
//   所以「头随波点头 50°」这个结论**不成立**，尺子自己有问题。
//
// 本轮的两处结构性改正
//   ① **空间判定只做一次**（`_bestSpace` 缓存），后续帧直接复用 —— 否则不同相位可能选到不同空间。
//   ② **顶点索引锁死**：在第 0 档（pitch=0）选好 head/core/tip/rear/top/bot 六组索引后就**不再重选**，
//      之后每帧只用这些索引取值。头簇是**刚性分支**，它只会整体转 ⇒ 吻部轴只会整体转，
//      仰角**应当**不随相位变。若仍变，那就是真有其事，而不是集合在抖。
//      （自检：锁死后 `fwdSpan` 应当逐档恒定 —— 报告里打出来，它是这套锁的"体温计"。）
//
// 判据（Pass A / A2 各 6 个相位）
//   · 仰角序列平滑且首尾相接 ⇒ 头确实随波点头，单点 0 不够，要动态修正
//   · 仰角序列基本平直（极差 < 3°）⇒ 头是刚性的，单点 pitch 就能定死，用户给的"0"直接可用
//   · fwdSpan 逐档恒定 ⇒ 索引锁生效；若漂 ⇒ 本表作废
//
// 出图：十档**绝对** pitch（+3 一路到 -24，步长 3），标签写绝对档位，用户指着哪张我就设哪个数。

public class drg_headscan22 : MonoBehaviour
{
    const string DIR = "D:/Unity Project/InkWash/Tools/screenshots/enemies/H22";
    const string RP = "D:/Unity Project/InkWash/Tools/reports/drg_headscan22.txt";
    const string KEY = "墨龙";
    const int W = 900, H = 600;
    const float DRIVE_HZ = 0.45f;

    static readonly float[] PHASES = new float[] { 0.00f, 0.1667f, 0.3333f, 0.50f, 0.6667f, 0.8333f };
    static readonly float[] ABS = new float[] { 3f, 0f, -3f, -6f, -9f, -12f, -15f, -18f, -21f, -24f };
    const float P_LEVEL = -12f;     // 第二十二轮候选档，Pass A2 复测它在 6 相位下的稳定性
    ...
    IEnumerator Run()
    {
        ...
        _sb.AppendLine("=== drg_headscan22：十档定档扫描（第二十二轮）===");
        ...
        _camS = MakeCam("PROBE_CAM_H22S");
        _camQ = MakeCam("PROBE_CAM_H22Q");
        float k = rad / Mathf.Sin(_camS.fieldOfView * 0.5f * Mathf.Deg2Rad);
        _dist = k; _distQ = k * 1.18f;
        AimSide(); AimQ();
        ...
        _sb.AppendLine("── Pass B：绝对 pitch 六档出图（同一行波相位 0.50）──");
        ...
        foreach (float p in ABS)
        {
            ...
            Bake(); Measure();
            string tag = Tag(p);
            File.WriteAllBytes(DIR + "/h22_side_" + tag + ".png", Shot(_camS));
            File.WriteAllBytes(DIR + "/h22_q_" + tag + ".png", Shot(_camQ));
        ...
        _sb.AppendLine("  · Pass A/A2 若极差 < 3° ⇒ 头是**刚性**的，单点 pitch 就够，用户给一个 0 即可定死。");
        _sb.AppendLine("  · Pass A/A2 若极差 > 10° 且序列平滑 ⇒ 头真的随波点头，要**动态**抵消，单点 0 不够。");
        _sb.AppendLine("  · 前向跨度逐档恒定 ⇒ 索引锁生效（本表的可信度体温计）。");
    ...
    IEnumerator Done()
    {
        ...
        Debug.Log("[drg_headscan22] 写入 " + RP);
        yield return null;
    }
}

var __hostH22 = new GameObject("drg_headscan22");
__hostH22.AddComponent<drg_headscan22>();
return "DRG_HEADSCAN22_STARTED";
