// q_demo_v4.cs —— 本轮修复的演示录屏（时间驱动；结尾还原姿态与鼠标）
// 相对 q_demo_v3 的变化：
//   ① 持剑待机换成 Baked/Idle_Carry_A（自然站姿 + 剑斜举，修掉「跨立太丑」）
//   ② 跑步换成 Baked/Run04_KI_Carry（右脚 Twist 从 [-120°,-26°] 校正到 [0.6°,8.5°]，修掉「脚 90° 旋转」）
//   ③ 新增「走路」段（非战斗待机→走路→跑步，比较三个状态的贴地）
//
//  0.0~1.6s  非战斗待机（垂手，剑在背）
//  1.6~2.0s  切战斗姿态（拔剑）
//  2.0~4.4s  持剑待机（Idle_Carry_A —— 两腿并拢，不再跨立）★
//  4.4~7.0s  走路（Walk）
//  7.0~10.6s 跑步（Run04_KI_Carry —— 脚不再被拧 90°、整体不再悬空）★
// 10.6~11.6s 停下，回到持剑待机
// 11.6~14.8s 三段连击（顺带展示左手握拳补丁）
...
IEnumerator Body()
{
    ...
    sb.AppendLine("=== 演示录屏 q_demo_v4（片段：持剑待机 Idle_Carry_A / 跑步 Run04_KI_Carry）===");
    sb.AppendLine("liftSmooth = " + (ik != null ? ik.liftSmooth.ToString("F1") : "?"));
    sb.AppendLine();
    ...
    sb.AppendLine(Seg("4 走路", 2.6f, stance, anim, ctl));
    ...
    sb.AppendLine(Seg("5 跑步 ★", 3.6f, stance, anim, ctl));

    // 6) 停下
    if (ctl != null) ctl.SetInjectedMove(Vector2.zero, false);
    t = Time.time; while (Time.time - t < 1.0f) yield return null;
    sb.AppendLine(Seg("6 停回待机", 1.0f, stance, anim, ctl));
    ...
    sb.AppendLine(Seg("7 三段连击", 3.2f, stance, anim, ctl));
    ...
    File.WriteAllText(Path.Combine(projRoot, "Tools/reports/q_demo_v4.txt"), sb.ToString());
    Debug.Log("[q_demo_v4] done");
    yield return null;
}
