// v39i_test.cs —— B 组终测：等敌 → dash逐帧 → 杀光 → 门涡+门下墨渍
...
IEnumerator Body()
{
    ...
    Debug.Log("[v39i] step-enemy-found"); sb.AppendLine("enemy = " + (enemy != null ? enemy.name : "NULL"));
    if (enemy == null) { Debug.Log("[v39i]\n" + sb); yield break; }
    ...
    if (pc != null && tr != null)
    {
        Debug.Log("[v39i] step-dash-inject"); pc.BeginInputOverride();
        pc.RequestInjectedDash(); Debug.Log("[v39i] step-dash-injected");
        var seq = "";
        for (int i = 0; i < 12; i++)
        {
            yield return null;
    ...
    var roomC = run.room;
    while (sw1.Elapsed.TotalSeconds < 60.0)
    {
        if (roomC != null && roomC.IsCleared) break;   // 波次全清（以刷怪器口径）
        foreach (var e in Object.FindObjectsOfType<InkWash.Enemies.EnemyBase>())
            if (e.IsAlive) e.TakeDamage(InkWash.Combat.DamageInfo.Simple(80f, InkWash.Combat.Faction.Player, null));
        yield return null;
    }
    Debug.Log("[v39i] step-killed"); sb.AppendLine("杀光耗时=" + sw1.ElapsedMilliseconds + "ms");
    ...
    Debug.Log("[v39i] step-door-wait-done"); yield return new WaitForSecondsRealtime(0.2f);
    run.DoorOpened -= onDoor;
